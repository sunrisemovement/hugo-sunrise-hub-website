import { DATA, signedpad, pad } from './Shared'
import { OffsetDateTime } from './OffsetDateTime'

export class Instant {
  [Symbol.toStringTag] = 'Instant'
  private [DATA]: { ms: number, ns: number }
  constructor(epochNanos: bigint) {
  ...
  withOffset(offsetString: string) {
  ...
  static fromEpochNanoseconds(epochNanos: bigint) {
    return new Instant(epochNanos);
  }
  static fromEpochMicroseconds(epochMicros: bigint) {
    return new Instant(epochMicros * BigInt(1e3));
  }
  static fromEpochMilliseconds(epochMillis: number) {
...
export function compare(one: Instant, two: Instant) {
